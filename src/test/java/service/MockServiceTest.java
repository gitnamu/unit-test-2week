...
public class MockServiceTest {
    ...
    // 1-1 어떤 포지션을 set 해도 미드를 리턴하도록 테스트 하세요.
    @Test
    public void 어떤포지션을set해도미드리턴테스트(){
        Champion champion = mock(Champion.class);
...
//        champion.setName("조이");
    ...
    public void 리산드라검색시포지션이탑이맞는지확인(){
        when(mockService.findByName("리산드라")).thenReturn(new Champion("리산드라","탑", 4));
    ...
    @Test
    public void 두개이상챔피언리스트로만들어호출시갯수확인(){
        List<Champion> champions = new ArrayList<>();

        champions.add(new Champion("르블랑","미드", 1));
        champions.add(new Champion("애쉬","바텀", 2));
        champions.add(new Champion("우르곳","탑", 3));
    ...
    @Test
    public void 가짜챔피언객체리턴및메소드한번호출되었는지검증및스킨개수확인(){
